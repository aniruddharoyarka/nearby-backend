...
const validate = (body) => {
 const data = {};
 for (const key of ["title", "category", "location", "date", "time", "validUntil", "description", "redemption"]) {
  if (typeof body[key] !== "string" || !body[key].trim()) throw new Error(key + " is required.");
  data[key] = body[key].trim();
 }
 for (const key of ["date", "validUntil"]) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(data[key]) || !Number.isFinite(Date.parse(data[key])) || new Date(data[key]).toISOString().slice(0,10) !== data[key]) throw new Error("Choose a valid date.");
 }
 if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(data.time)) throw new Error("Choose a valid time.");
 if (data.validUntil < data.date || data.validUntil < today()) throw new Error("Expiry must be on or after the start date and today.");
 for (const key of ["originalPrice", "discountPercent"]) {
  if (!["number", "string"].includes(typeof body[key]) || String(body[key]).trim() === "") throw new Error("Price and discount are required.");
  data[key] = Number(body[key]);
  if (!Number.isFinite(data[key]) || data[key] < 0) throw new Error("Price and discount must be non-negative numbers.");
 }
...
const list = (scope) => async (req,res) => {
 try {
  const filter = scope === "admin" ? {} : scope === "mine" ? { organizer: req.user.userId } : publicFilter();
  const offers = await Offer.find(filter).populate("organizer", populate).sort({createdAt:-1});
  res.json({ offers: offers.map(serialize) });
 } catch { res.status(500).json({message:"Failed to load offers."}); }
};
const detail = (mine = false) => async (req,res) => {
 if (!mongoose.isObjectIdOrHexString(req.params.id)) return res.status(404).json({message:"Offer not found."});
 try {
  const offer = await Offer.findOne({ _id:req.params.id, ...(mine ? {organizer:req.user.userId} : publicFilter()) }).populate("organizer",populate);
  if (!offer) return res.status(404).json({message:"Offer not found."});
  res.json({offer:serialize(offer)});
 } catch { res.status(500).json({message:"Failed to load offer."}); }
};
const save = (edit = false) => async (req,res) => {
 if (edit && !mongoose.isObjectIdOrHexString(req.params.id)) return res.status(404).json({message:"Offer not found."});
 let data;
...
const remove = async (req,res) => {
 if (!mongoose.isObjectIdOrHexString(req.params.id)) return res.status(404).json({message:"Offer not found."});
 try {
  const offer=await Offer.findOneAndDelete({_id:req.params.id,organizer:req.user.userId});
  if (!offer) return res.status(404).json({message:"Offer not found."});
...
module.exports={list,detail,save,remove,moderate,validate,serialize,publicFilter};
